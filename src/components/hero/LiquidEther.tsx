...
type FBO = THREE.WebGLRenderTarget | null;
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const bgVec4 = new THREE.Vector4(0, 0, 0, 0); // transparente
...
void main(){
  ...
  gl_FragColor = vec4(newv, 0.0, 0.0);
    ...
    class Advection extends ShaderPass {
        ...
        super({
          material: {
            ...
            uniforms: {
              ...
              velocity: { value: simProps.src ? simProps.src.texture : null },
      ...
      updateParams(args: { dt?: number; isBounce?: boolean; BFECC?: boolean }) {
  ...
  constructor(simProps: { cellScale: THREE.Vector2; cursor_size: number; dst: FBO }) {
    ...
    this.cursorSize = simProps.cursor_size;
    this.init();
  }

  // ← sin argumentos, coincide con la base
  init(): void {
    super.init();
    ...
    const mouseM = new THREE.RawShaderMaterial({
      ...
      uniforms: {
        ...
        scale: { value: new THREE.Vector2(this.cursorSize, this.cursorSize) }
      }
    });
  ...
  updateProps(props: { mouse_force: number; cursor_size: number; cellScale: THREE.Vector2 }) {
    const forceX = (Mouse.diff.x / 2) * (props.mouse_force || 0);
    const forceY = (Mouse.diff.y / 2) * (props.mouse_force || 0);
    const { cellScale } = props;
  ...
  override update(): void {
    ...
    class Viscous extends ShaderPass {
        ...
        super({
          material: {
            ...
            uniforms: {
              ...
              velocity: { value: simProps.src ? simProps.src.texture : null },
              velocity_new: { value: simProps.dst_ ? simProps.dst_.texture : null },
      ...
      step(iterations: number, dtVal: number): FBO {
        if (!this.uniforms) return this.props.output ?? null;
        let fbo_in: FBO = this.props.output0 ?? null;
        let fbo_out: FBO = this.props.output1 ?? null;
        for (let i = 0; i < iterations; i++) {
          ...
          this.uniforms.velocity_new.value = fbo_in ? fbo_in.texture : null;
          this.props.output = fbo_out ?? null;
      ...
      setViscous(v: number) {
        if (this.uniforms) this.uniforms.v.value = v;
      }
    }
    ...
    class Divergence extends ShaderPass {
      ...
      setVelocity(vel: FBO) {
        if (this.uniforms) this.uniforms.velocity.value = vel ? vel.texture : null;
      }
    }
    ...
    class Poisson extends ShaderPass {
      constructor(simProps: {
        ...
        super({
          material: {
            ...
            uniforms: {
              boundarySpace: { value: simProps.boundarySpace },
              pressure: { value: simProps.dst_ ? simProps.dst_.texture : null },
              divergence: { value: simProps.src ? simProps.src.texture : null },
              px: { value: simProps.cellScale },
            },
          },
          output: simProps.dst,
      ...
      iterate(iterations: number): FBO {
        let p_in: FBO = this.props.output0 ?? null;
        let p_out: FBO = this.props.output1 ?? null;
        for (let i = 0; i < iterations; i++) {
          if (i % 2 === 0) {
          ...
          if (this.uniforms) this.uniforms.pressure.value = p_in ? p_in.texture : null;
          this.props.output = p_out ?? null;
          super.update();
        }
        return p_out ?? null;
      }
    ...
    class Pressure extends ShaderPass {
      constructor(simProps: {
        ...
        super({
          material: {
            ...
            uniforms: {
              boundarySpace: { value: simProps.boundarySpace },
              pressure: { value: simProps.src_p ? simProps.src_p.texture : null },
              velocity: { value: simProps.src_v ? simProps.src_v.texture : null },
              px: { value: simProps.cellScale },
              dt: { value: simProps.dt },
            },
          },
      ...
      setSources(vel: FBO, pressure: FBO) {
        if (!this.uniforms) return;
        this.uniforms.velocity.value = vel ? vel.texture : null;
        this.uniforms.pressure.value = pressure ? pressure.texture : null;
      }
    }
    ...
    class Simulation {
      ...
      update() {
        ...
        this.externalForce.updateProps({
          cursor_size: this.options.cursor_size,
        ...
        if (this.options.isViscous) {
          this.viscous.setViscous(this.options.viscous);
          vel = this.viscous.step(this.options.iterations_viscous, this.options.dt);
        }

        this.divergence.setVelocity(vel);
        this.divergence.update();

        const pressure = this.poisson.iterate(this.options.iterations_poisson);
        this.pressure.setSources(vel, pressure);
      ...
      constructor() {
        ...
        this.output = new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({
            ...
            depthWrite: false,
            uniforms: {
              velocity: { value: this.simulation.fbos.vel_0 ? this.simulation.fbos.vel_0.texture : null },
  ...
  // actualización cuando cambian props
  useEffect(() => {
