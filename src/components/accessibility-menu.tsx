...
export default function AccessibilityMenu() {
  ...
  useEffect(() => {
    const onOverlayOpen = (event: Event) => {
      ...
      if (source !== 'accessibility') setOpen(false);
    };
  ...
  useEffect(() => {
    if (!open) return;
    window.dispatchEvent(new CustomEvent(OVERLAY_EVENT, { detail: 'accessibility' }));
  }, [open]);
    ...
    <>
      {open && (
        <>
          ...
          <aside
            className="fixed inset-x-3 bottom-20 z-[71] rounded-2xl border border-token bg-[color:var(--bg)]/96 p-4 shadow-2xl backdrop-blur-xl sm:bottom-24 sm:left-4 sm:right-auto sm:w-[calc(100vw-2rem)] sm:max-w-xs"
          ...
          >
            ...
            <div className="grid gap-3">
              <button
                ...
                className="inline-flex items-center justify-between rounded-xl border border-token bg-[color:var(--surface)] px-3 py-2 text-sm transition hover:bg-[color:var(--surface-2)]"
      ...
      <button
        ...
        className={`fixed bottom-5 left-4 z-[71] inline-flex h-12 w-12 items-center justify-center rounded-full border border-token bg-[color:var(--surface)] text-[var(--fg)] shadow-xl transition hover:bg-[color:var(--surface-2)] ${open ? 'pointer-events-none opacity-0' : ''}`}
        aria-expanded={open}
