  ...
  return (
    <section className="container py-16" id="proceso">
      ...
      <div ref={ref} className="relative mt-10">
        {/* Timeline central (light/dark aware) */}
            ...
            return (
              <div key={s.n} className={`grid gap-4 sm:grid-cols-2 ${left ? 'sm:text-right' : 'sm:text-left'}`}>
                ...
                <motion.div variants={variants} className={`${left ? '' : 'sm:order-2'}`}>
                  <div
                    className="group relative inline-block rounded-2xl border bg-[color:var(--surface)] px-6 py-5
                               backdrop-blur shadow-token transition hover:-translate-y-1"
                  ...
                  >
                    ...
                    <motion.span
                      ...
                      style={{
                        ...
                        boxShadow:
                          '0 6px 18px color-mix(in oklab, var(--acc-indigo) 22%, transparent)',
                      }}
                    >
                      {s.n}
                    </motion.span>
                    ...
                    <div
                      ...
                      style={{
                        background:
                          'linear-gradient(180deg, color-mix(in oklab, var(--acc-indigo) 70%, white 0%), color-mix(in oklab, var(--acc-cyan) 65%, var(--acc-indigo)))',
                        boxShadow:
                          '0 6px 16px color-mix(in oklab, var(--acc-cyan) 22%, transparent)',
                        // ring sobrio
                        // @ts-ignore – tailwind inline unneeded: usamos estilo directo
                        borderColor: 'transparent',
                        // “ring” con color-mix
                        // (simula ring-teal-500/20 pero con nuestra paleta)
                        // Nota: ring usa shadow para suavidad visual
                      }}
                    />
                  </motion.div>
                </div>
