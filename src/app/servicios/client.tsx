...
const icons: Record<string, JSX.Element> = {
  'web-movil': (
    <svg width="22" height="22" viewBox="0 0 24 24" className="opacity-90" aria-hidden>
...
const accents: Record<
  string,
  { pill: string; border: string; shine: string }
> = {
  'web-movil': {
...
export default function ServiciosClient() {
  const { lang } = useI18n();
  const t = copy[lang];
  const items = servicesIndexByLang(lang); // deja solo los 4 servicios recortados en tu fuente de contenidos
    ...
    <main key={lang} className="container py-16">
      ...
      >
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-300">
          ...
          <span className="rounded-full bg-white/5 px-3 py-1 ring-1 ring-white/10">
            {lang === 'es' ? 'Respuesta en 24h' : '24h response'}
          </span>
          <span className="rounded-full bg-white/5 px-3 py-1 ring-1 ring-white/10">
            {lang === 'es' ? 'Factura disponible' : 'Invoice available'}
          </span>
        </div>
      </motion.section>
              ...
              >
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between">
                    <span className={`rounded-xl ${acc.pill} p-2.5 ring-1`}>{icons[s.slug]}</span>
      ...
      {/* Bloque de confianza minimal (sin números inflados) */}
        ...
        <div className="rounded-2xl bg-white/[0.04] p-6 ring-1 ring-white/10">
          <div className="grid gap-4 md:grid-cols-3">
            {t.trust.map((m) => (
              ...
              >
                <div className="text-sm font-semibold text-[var(--ac-accent)]">{m.k}</div>
